...
object Nursery {
  ...
  def apply[F[_]: Concurrent]: Resource[F, Nursery[F]] =
    Resource
      .liftF((Ref[F].of(0), Ref.of[F, List[F[Unit]]](Nil)).tupled)
      .flatMap {
        case (fibersCount, cancelTokens) =>
          Resource.makeCase[F, Nursery[F]] {
            val nursery = new Nursery[F] {
              def startSoon[A](fa: F[A]): F[Fiber[F, A]] =
                fibersCount
                  .update(_ + 1)
                  .bracket(_ => fa)(_ => fibersCount.update(_ - 1))
                  .guaranteeCase {
                    case ExitCase.Error(ex) => cancelAllFibers(cancelTokens)
                    case _                  => ().pure[F]
                  }
                  .start
            ...
            case (_, ExitCase.Completed) => waitAllFibers(fibersCount)
            case (_, _)                  => cancelAllFibers(cancelTokens)
          }
      }
}
...
object Main2 extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] =
    happyPath[IO] >> IO(println("happyPath done")) >>
      nestedUsage[IO] >> IO(println("nestedUsage done")) >>
  ...
  def happyPath[F[_]: Concurrent: Timer] = test[F] { n =>
    for {
      d1 <- Deferred[F, Unit]
      d2 <- Deferred[F, Unit]
